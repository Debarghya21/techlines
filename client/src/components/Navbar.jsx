...
const Navbar = () => {
  ...
  return (
    <Box bg={useColorModeValue('gray.100','gray.900')} px={4}>
            ...
            <HStack as='nav' spacing={4} display={{base:'none',md:'flex'}}>
                {
                    links.map((link) => (
                        <NavLink
                        key={link.linkName} path={link.path}>
                            {link.linkName}
                        </NavLink>
                    ))
                }
        ...
        <Button as={ReactLink} to='\registration' m={2} display={{base:'none',md:'inline-flex'}} fontSize='sm' fontWeight={600} _hover={{bg : 'orange.400'}} bg='orange.500' color='white'>
            Sign Up
        </Button>
        ...
        {isOpen ? <Box pb={4} display={{md:'none'}}>
            <Stack as='nav' spacing={4}>
            {
                ...
                <NavLink key='sign up' to='\registration'>Sign Up</NavLink>  
            </Stack>
             </Box>:null}
